            ...
            <div class="col-9 mx-auto">
                <div class="card card-body mt-4">
                    ...
                    <hr class="horizontal dark my-3">
                        <form action="/laravel-edit-role/{{$role->id}}" method="POST">
                            @csrf
                            <div>
                                <label for="rolesName" class="form-label">Role Name</label>
                                <div class="">
                                    <input type="text" class="form-control " id="roleName" name="name" onfocus="focused(this)" onfocusout="defocused(this)" value="{{$role->name}}">
                                    @error('name')
                            ...
                            <div>
                                ...
                                <div class="">
                                    <textarea type="text" class="form-control  " name="description" id="roleDescription">{{$role->description}}</textarea>
                                    @error('description')
                                        <p class="text-danger text-xs mt-2 mb-2">{{ $message }}</p>
                                    @enderror
                                </div>
